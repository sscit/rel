...
ParseResult RelParser::LogParseException(ParseException &e) {
    ...
    l.LOG(LogLevel::ERROR,
          *t.GetFilename() + ": Line " +
              std::to_string(t.GetLineNumberOfToken()) + ", Pos " +
              std::to_string(t.GetPositionInLineOfToken()) + ":");
    l.LOG(LogLevel::ERROR, e.what());
...
void RelParser::LexFile(FileTokenData &file) {
    l.LOG(LogLevel::DBUG,
          "Reading file " + file.filepath + " and creating tokens");
    Lexer lex(l);
    lex.Read(file);
}

ParseResult RelParser::ReadAndParseDataFiles(
    std::vector<FileTokenData> &input_files) {
    l.LOG(LogLevel::INFO, "Processing data files...");
    /* Define the function that is spawned into a thread. There will be N
       threads, one for every data file. To report back if there are exceptions
       during parsing, an exception pointer is provided. */
    std::vector<std::exception_ptr> eps;
    auto thread_main = [&](FileTokenData *const input,
    ...
    while ((iter = std::find_if(iter, input_files.end(), [](FileTokenData &d) {
                return (d.GetDataTypeOfTokenList() ==
                        DataType::RequirementsData);
            })) != input_files.end()) {
        /* Provide the index of the file, to ensure that the resulting type
           instances are stored in the same
           order in the data base as the files have been got out of the sorted
    ...
    /* Check if there was an exception during data parsing. if yes, just output
     * the first one */
    if (eps.size() > 0) {
        try {
...
void RelParser::ReadAndParseSpecFiles(std::vector<FileTokenData> &input_files) {
    ...
    while ((iter = std::find_if(iter, input_files.end(), [](FileTokenData &d) {
                return (d.GetDataTypeOfTokenList() ==
                        DataType::RequirementsSpecification);
            })) != input_files.end()) {
        LexFile(*iter);
...
std::list<RdFile> RelParser::GetDatabase() { return rd_parser.GetDatabase(); }