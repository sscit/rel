        ...
        for (unsigned int j = 0; j < t.attributes.size(); j++) {
            ...
            if (te.token_of_attribute.GetTokenType() == TokenType::ENUM) {
                ...
                if (search_result != all_enums.end()) {
                    ...
                    throw EnumUsedButNotDefinedException(
                        te.token_of_attribute, "Enum " +
                                                   te.enum_definition.Get() +
...
template <class T>
void RsParser::EnsureToken(FileTokenData const& tokens,
...
template <class T>
void RsParser::CleanupDatabase(std::string const& path,
    ...
    for (auto const& to : origin) {
        if (to.path.compare(path) == 0) all.erase(to.type_name);
    }
    origin.remove_if(
    ...
    while (iter != tokens.token_list.end()) {
        ...
        if (current_token.GetTokenType() == TokenType::LINE_COMMENT) {
            ...
            try {
                ...
                l.LOG(LogLevel::DBUG,
                      "Enumeration not ready to be parsed completely, hit "
                      "Array out of bounds.");
                throw RsEnumException(current_token,
                                      "Enum definition incomplete");
            }
            ...
            try {
                ...
                l.LOG(LogLevel::DBUG,
                      "Type definition not ready to be parsed completely, hit "
                      "Array out of bounds.");
                throw RsTypeException(current_token,
...
bool RsParser::IsEnumValueUnique(RsEnum const& enum_type,
                                 RsRdIdentifier const& enum_value) {
    bool ret = true;

    std::vector<RsRdIdentifier>::const_iterator iter =
        enum_type.enum_elements.begin();
...
RsEnum RsParser::EnumDefinition(FileTokenData const& tokens,
                                std::list<Token>::const_iterator& iter) {
    ...
    if (iter->GetTokenType() == TokenType::IDENTIFIER) {
        ...
        iter++;
        EnsureToken(tokens, iter, TokenType::BRACKET_OPEN,
                    RsEnumException(SafeDeref(tokens, iter),
        ...
        while (!IsNextToken(tokens, iter, TokenType::BRACKET_CLOSE)) {
            ...
            if (IsEnumValueUnique(enumeration, enum_value))
                ...
                throw RsEnumException(*iter, "Enum value " + enum_value.name +
                                                 " already defined");
            iter++;
        ...
        if (all_enums.find(enumeration.Get()) != all_enums.end())
            throw RsEnumException(enum_token,
                                  "Enum with this name already defined");
    } else {
        throw RsEnumException(enum_token, "Wrong token, expected identifier");
    }

    l.LOG(LogLevel::DBUG, "Enumeration " + enumeration.Get() + " (" +
                              std::to_string(enumeration.enum_elements.size()) +
                              " elements) has been created.");

    return enumeration;
}

bool RsParser::IsAttributeNameUnique(RsType const& type,
                                     RsRdIdentifier const& new_attribute) {
    bool ret = true;
...
RsType RsParser::TypeDefinition(FileTokenData const& tokens,
                                std::list<Token>::const_iterator& iter) {
    ...
    if (iter->GetTokenType() == TokenType::IDENTIFIER) {
        ...
        iter++;
        EnsureToken(tokens, iter, TokenType::BRACKET_OPEN,
                    RsTypeException(SafeDeref(tokens, iter),
                                    "Wrong token, expected {"));
        iter++;
        ...
        while (!IsNextToken(tokens, iter, TokenType::BRACKET_CLOSE)) {
            ...
            if (IsAttributeNameUnique(type, type_attribute.name) == false) {
                throw RsTypeException(SafeDeref(tokens, iter),
                                      "Attribute name " +
                                          type_attribute.name.Get() +
                                          " already used.");
            }
            iter++;
            ...
            if (iter->GetTokenType() == TokenType::ID ||
                ...
                Token enum_token(*tmp.GetTokenValue(), TokenType::ENUM,
                                 *tmp.GetFilename(), tmp.GetLineNumberOfToken(),
                                 tmp.GetPositionInLineOfToken());
        ...
        if (all_types.find(type.Get()) != all_types.end())
            throw RsTypeException(type_token,
                                  "Type with this name already defined");
    } else {
        throw RsTypeException(type_token, "Wrong token, expected identifier");
    }

    l.LOG(LogLevel::DBUG, "Type " + type.Get() + " (" +
                              std::to_string(type.attributes.size()) +
                              " elements) has been created.");
