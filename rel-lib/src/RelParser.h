...
enum class ParseResult { NoExceptionOccurred = 0, ExceptionOccurred = 1 };

/* This class implements a workflow, which uses Lexer and Parser of
   REL specification and REL data to process a REL model.

   It requires an object of class Logger and an object of FileEngine, to hand
   over the relevant files that shall be parsed. After execution, the resulting
   database containing all requirements can be retrieved. */
class RelParser {
   public:
