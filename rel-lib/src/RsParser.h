...
class RsParser : public Parser {
   ...
   protected:
     ...
     * Req: dsl3
     */
    RsEnum EnumDefinition(FileTokenData const &,
                          std::list<Token>::const_iterator &);
    /* Parse type definition out of AST
     * Req: dsl2
     */
    RsType TypeDefinition(FileTokenData const &,
                          std::list<Token>::const_iterator &);
     ...
     */
    template <class T>
    void CleanupDatabase(std::string const &, std::map<std::string, T> &,
                         std::list<TypeOrigin> &);
    ...
    void AddToDatabase(T const &, std::string const &,
                       std::map<std::string, T> &, std::list<TypeOrigin> &);
    ...
    // contains data structure that maps every key to the URI where it comes
    // from
