...
void LspEngine::RespondToDefinition(json const& input_message) {
    ...
    json response_message{{"id", input_message["id"]}, {"result", ""}};
    ...
    if (ws.GetTargetOfLink(current_file.GetPath(), link, target, target_path)) {
        ...
        response_message = {{"id", input_message["id"]}, {"result", location}};
    }
...
void LspEngine::HandleMessage(json const input_message) {
    if (input_message["method"] == "initialize") {
        ...
        if ((current_file.IsRequirementsData() ||
             current_file.IsRequirementsSpecification()) &&
            text.size() > 0) {
            l.LOG(LogLevel::DBUG, "REL document " + current_file.GetPath() +
                                      " has been opened or changed");
...
void LspEngine::ParseDocument(Uri const& uri, std::string const& text) {
    ...
    json diag_message{{"method", "textDocument/publishDiagnostics"},
                      {"params",
                       {
...
void LspEngine::SendMessageToClient(json const& message_body) {
    ...
    header.append("Content-Length: " +
                  std::to_string(full_message.dump(4).size()) + "\r\n");
